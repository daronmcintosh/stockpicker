...
output "deployment_instructions" {
  ...
  value = <<-EOT
    Your production droplet is ready! Here's how to deploy:
    ...
    5. For production, set up a reverse proxy (nginx/Caddy) for HTTPS.
    ...
    - Only expose port 80/443, not individual service ports
  EOT
}
